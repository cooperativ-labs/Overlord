...
async function requestDeviceCode(platformUrl) {
...
async function pollDeviceCode(platformUrl, deviceCode) {
...
export async function authLogin() {
  ...
  const { device_code, user_code, verification_uri, expires_in } =
    await requestDeviceCode(platformUrl);
  ...
  while (Date.now() < deadline) {
    ...
    const result = await pollDeviceCode(platformUrl, device_code);
