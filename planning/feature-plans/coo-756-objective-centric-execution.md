...
1. **Objective display-id separator is `.`** (follow-up 2026-08-17). Public form is `coo:756.k7xm`, not `coo:756|k7xm`. `|` needs shell quoting and is outside the desktop deep-link charset. `-` is already used inside workspace slugs (`my-team:12`), so the objective boundary would be hard to see. `.` is unquoted-safe in shells, URI-unreserved, filename-safe, and the only dot in the string (slugs are `[a-z0-9-]+`). `:` remains the workspace-slug → mission-sequence separator.
...
Examples: `coo:756.k7xm`, `my-team:12.n4w2`.
...
- No shell quoting, URL encoding, or filename sanitizing for the separator.

Separator is `.`, not `:` (that already splits slug from sequence) and not `|` or `-` (see Decisions).
...
- `coo:756|k7xm` (`|`) — pipes in bash/zsh unless quoted; not in the desktop deep-link charset `[A-Za-z0-9:_-]{1,64}`; breaks markdown tables; awkward in filenames.
- `coo:756-k7xm` (`-`) — workspace slugs already contain hyphens (`my-team:12-k7xm` has three `-` characters).
- `coo:756:k7xm` (`:`) — collides with `slug:sequence`.
- `coo:756.1` / `coo:756.2` — the **dot is fine**; a **decimal sequence** is not (implies current order). The key must stay a random Crockford string.
...
1. Objective UUID.
2. Full display id matching `^(.+):(\d+)\.([0-9a-hjkmnp-tv-z]{4})$` (case-insensitive key; normalize to lowercase). The only `.` in a well-formed id is this separator: slugs are `[a-z0-9-]+` and never contain dots.
...
- `coo:756:k7xm`, `coo:756|k7xm`, `coo:756-k7xm` (wrong separator).
...
and instruct agents to pass `--mission-id coo:756` **and** `--objective-id coo:756.k7xm` on attach when both exist. Until protocol attach accepts `--objective-id`, the launch wrapper must pass it (env + argv), not rely on the model. Generated copy-paste commands do **not** need quotes around the objective id.
...
### 4.7 Shell, files, and URLs

`.` does not need escaping in bash/zsh unquoted words, is URI-unreserved, and is legal in filenames. Generated CLI examples should be:

```
ovld protocol attach --mission-id coo:756 --objective-id coo:756.k7xm
```

Context files on disk should still avoid putting the raw display id in the path if a future key alphabet ever grows punctuation; with Crockford keys, `objective-coo-756-k7xm.md` (hyphens) remains a good tmp filename. The **id string** itself uses `.`.

Deep links: existing `overlord://missions/<id>` charset is `[A-Za-z0-9:_-]{1,64}`. Phase C should **add `.`** so an objective display id can appear in a query or path without encoding. Do not reuse `|`. See §9.6.
...
- Title line can remain mission for the file name `mission-coo-756.md` **or** become `mission-coo-756-k7xm.md`. **Recommendation:** `objective-coo-756-k7xm.md` (hyphens in the **path**; the id string is `coo:756.k7xm`).
...
`.overlord/tmp/mission-<displayId>.md` — prefer `objective-coo-756-k7xm.md` (hyphens in the path). Keep writing under `.overlord/tmp/`.
...
Desktop handler currently allows mission ids matching `[A-Za-z0-9:_-]{1,64}`. **Add `.`** to that class (or to the objective-specific route) so `coo:756.k7xm` does not need encoding. Do not silently drop objective URLs (that was the coo:502 bug for missions).
...
- Context file and copied prompt contain unquoted `--objective-id coo:….xxxx`
...
2. **Deep-link charset** today is `[A-Za-z0-9:_-]{1,64}` and omits `.`. Mitigation: Phase C adds `.`; CLI, env, Latch names, and REST work without that change.
...
Record decisions in a follow-up before Phase D; A/B can proceed on the recommendations below. **Display-id separator is decided (`.`)** — see Decisions.
