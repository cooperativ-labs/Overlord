...
import { getActorWorkspaceUserId, requireDatabaseClient } from './db.ts';
...
export async function postProjectRunQueue(projectId: string, body: { name?: string }) {
  const db = requireDatabaseClient();
  await authorize(projectId, PERMISSIONS.PROJECT_UPDATE, db);
  return createQueue(db, projectId, body.name ?? '', getActorWorkspaceUserId());
}
...
export async function deleteRunQueueEntry(entryId: string) {
  const db = requireDatabaseClient();
  const projectId = await projectForEntry(db, entryId);
  await authorize(projectId, PERMISSIONS.EXECUTION_REQUEST_CREATE, db);
