...
`--mission-id` is required. Pass `--objective-id` (UUID or display id such as
`coo:312.k7xm`) when you already know which objective to execute — attach will
not rediscover another one. Runner launches export `OVERLORD_OBJECTIVE_ID` and
the CLI forwards it automatically. Mission-only attach still selects the current
active objective.
...
provenance. Hosted MCP exposes the same call as `overlord_add_artifact`.
