...
- `overlord_queue_objective` — add, move, or remove one objective from the project Run Queue; requires `objectiveId` (UUID or display id)
...
`overlord_queue_objective` accepts optional `queue` / `after` placement and
`remove: true` to dequeue. Queue membership is target-neutral and sequences
delivery-driven launches; it does not directly launch the objective.
