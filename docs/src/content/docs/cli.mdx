...
Use it when reconnecting to a mission that is running more than one objective —
there, the mission id alone no longer identifies a unit of work. An objective
UUID names no mission, so it still needs `--mission-id` beside it.
