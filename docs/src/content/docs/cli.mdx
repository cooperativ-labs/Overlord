...
| `ovld inputs list --mission-id <id> [--json]` | List session instructions queued or delivered to a mission's live channels. |
...
| `ovld agent-session bind --agent <key> --native-session-id <id> [--mission-id <id>]` | Correlate a harness-native session id with this process's channel. |
