...
| Command | What it does |
| --- | --- |
| `ovld requests [--json]` | List open and recent agent decisions (time-bounded permission requests). |
| `ovld requests resolve <id> --revision <n> --decision allow\|deny\|ask [--text <text>]` | Answer a held decision. |
| `ovld inputs list --mission-id <id> [--objective-id <id>] [--json]` | List session instructions queued or delivered to a mission's live channels. |
