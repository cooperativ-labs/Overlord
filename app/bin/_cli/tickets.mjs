...
export async function ticketsCreate(args) {
  ...
  const data = await apiPost(`${platformUrl}/api/protocol/tickets`, agentToken, body);
...
export async function ticketsList(args) {
  ...
  const data = await apiPost(`${platformUrl}/api/protocol/list-tickets`, agentToken, body);
