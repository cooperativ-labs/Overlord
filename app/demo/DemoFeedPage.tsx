      ...
      <div className="min-w-0 flex-1 pb-6">
        ...
        <div className="rounded-lg border bg-card p-4">
          <div className="mb-2 flex items-start gap-2">
            ...
            <h3
              className="flex-1 cursor-pointer text-sm font-semibold leading-snug"
              onClick={() => setExpanded(!expanded)}
            >
              {post.title}
            </h3>
            ...
            <div className="mt-2 ml-6 rounded-md border border-blue-200 bg-blue-50 px-3 py-2 dark:border-blue-800/40 dark:bg-blue-950/20">
              <ul className="space-y-0.5">
                {humanActions.slice(0, 3).map((action, index) => (
                  <li key={index} className="flex gap-2 text-xs text-blue-800 dark:text-blue-300">
              ...
              {filesTouched.length > 0 && (
                <div className="flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
                  ...
                  {filesTouched.slice(0, 5).map(file => (
                    <code key={file} className="rounded bg-muted px-1 py-0.5 text-[10px]">
                      {file}
                    </code>
                  ))}
