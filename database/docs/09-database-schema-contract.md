...
| `allow_parallel_objectives`    | Bool         | yes      | Opt-in permission for two objectives on this mission to execute at once when their effective `resource_key`s differ. Default `false` (serial sibling 409). Same-resource pairs remain forbidden until per-objective worktrees exist. Not a unique index; enforced in `launchObjective` and `validateObjectiveLifecycle`. |
