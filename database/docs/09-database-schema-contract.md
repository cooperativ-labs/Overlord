...
Adapters may add optimized indexes, generated columns, full-text indexes, triggers, or notification mechanisms as long as they preserve the logical contract. The Postgres adapter uses an `execution_requests` insert/update trigger to emit a queue wake hint when a row becomes `queued`; runner claim long-polls may LISTEN for that hint on a dedicated non-pooled connection, but must re-run normal scoped authorization and atomic claiming after every wake. A mirrored trigger emits a completion hint when a `local_target_mutation` row reaches a terminal status, which callers awaiting a capability result may LISTEN for on the same terms — a wake means "re-read the row", never "your result is ready". SQLite has no equivalent notification requirement and callers fall back to a bounded poll.
...
| `notes_text`                   | text         | no       | Human-only mission notes for viewers; never included in agent attach/load context.                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `active_branch`                | text         | no       | Git branch the mission is currently operating on under worktree automation; null until the first launch prepares one. A per-objective isolated branch (parallel objectives) is recorded on `objectives.branch` only and never moves this pointer.                                                                                                                                                                                                                                                                                               |
...
| `worktree_preference`          | text         | no       | Per-mission override of the user's `worktreeBranchAutomationEnabled` launch default (stored in `profiles.metadata_json`, not on the workspace). `null` inherits the user default; `'worktree'` forces a branch + worktree for this mission even when automation is off; `'branch'` forces a branch without a dedicated worktree (checked out in the project's primary repo). Persistent (not cleared by the runner). App-validated open set (no DB CHECK).                                                                                                                                                |
| `allow_parallel_objectives`    | Bool         | yes      | Opt-in permission for two objectives on this mission to execute at once, on the same `resource_key` or different ones. Default `false` (serial sibling 409). Checkout safety is the Runner Layer's job: with worktrees on, a concurrently launched same-resource objective is planned onto `<mission branch>-<objectives.display_key>` in its own worktree; with worktrees off, the mission has one checkout that concurrent objectives share deliberately. Not a unique index; enforced in `launchObjective` and `validateObjectiveLifecycle`. |
...
| `created_by_workspace_user_id` | Id           | no       | FK to `workspace_users`. For an agent-authored mission this is the human behind the authenticating token — the member the agent acted as, or on behalf of.                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `display_key`                  | text         | yes      | Stable Crockford Base32 key (lowercase alphabet `0123456789abcdefghjkmnpqrstvwxyz`, typically 4 characters; 5 on collision exhaustion). Unique among non-deleted rows of the same mission (`idx_objectives_mission_display_key`). Public id is computed at read time as `{missions.display_id}.{display_key}` (e.g. `coo:756.k7xm`) and is never persisted as a second column. Does not encode `position`. Soft-deleted keys may be reused. |
...
| `launched_at`                  | TimestampUTC | no       | When the objective first entered `launching`. First-wins (`COALESCE`): a re-launch never rewrites it.                                                                                                                                                                                                                                                                                                                                       |
| `started_at`                   | TimestampUTC | no       | When the objective first entered `executing`. First-wins: a post-delivery re-attach keeps the original.                                                                                                                                                                                                                                                                                                                                     |
| `completed_at`                 | TimestampUTC | no       | Set when state enters `complete`; cleared if reopened.                                                                                                                                                                                                                                                                                                                                                                                      |
| `reopened_at`                  | TimestampUTC | no       | Last-wins run boundary (contract v133). Stamped when an objective in `executing`, `pending_delivery`, or `complete` is set back to `draft`; rows of its deliveries, terminal sessions, and file changes at or after this moment belong to the latest run. Not stamped by a post-delivery re-attach, a `future → draft` promotion, or a `launching → draft` reset. Null until the first reopen.                                              |
...
| `created_by_workspace_user_id` | Id           | no       | FK to `workspace_users`. For an agent-authored objective this is the human behind the authenticating token.                                                                                                                                                                                                                                                                                                                                 |
| `created_by_kind`              | text         | yes      | Class of actor that authored the row: `human`, `agent`, or `automation` (closed vocabulary, enforced by CHECK). Defaults to `human`. Resolved independently of the mission's own value — a human mission gets agent objectives and vice versa.                                                                                                                                                                                              |
| `created_by_agent`             | text         | no       | Connector/agent identifier that authored the row (open vocabulary, e.g. `claude-code`). Distinct from `assigned_agent`, which names the agent that will _run_ the objective. Null for human and automation authorship.                                                                                                                                                                                                                      |
| `created_by_session_id`        | Id           | no       | Agent session the creating call ran inside. A **deliberate soft reference** to `agent_sessions.id` with **no foreign key**, so a dangling provenance pointer can never block a delete; resolve with a `LEFT JOIN` and tolerate a miss.                                                                                                                                                                                                      |
| `created_at`                   | TimestampUTC | yes      |                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `updated_at`                   | TimestampUTC | yes      |                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `deleted_at`                   | TimestampUTC | no       | Tombstone.                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `revision`                     | integer      | yes      |                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `type`                    | text         | yes      | `update`, `user_follow_up`, `alert`, `discussion_summary`, `decision`, `ask`, `answer`, `permission_request`, `delivery`, `execution_requested`, `awaiting_approval`, `status_change`. |
...
| `payload_json`                   | Json         | yes      | Structured delivery payload. Versioned `deliveryReport` stores normalized evidence and deterministic presentation; `summary` remains immutable agent-authored source text. Raw payload JSON is never a REST response; `GET /api/missions/:id/deliveries` returns a `MissionDeliveriesDto` page of normalized `DeliveryDto` items and synthesizes a deterministic V1 report for legacy records. |
...
Project-scoped named execution sequences. Exactly one live row per project may have `is_default`; each queue owns its display `position`, `paused` state, name, revision, creation actor, and soft-delete lifecycle. Queues do not select targets or constrain project-wide concurrency.

### `run_queue_entries`

Authoritative Run Queue membership. Each live objective appears at most once across its project's queues. Rows carry queue/project/workspace/mission/objective identity, mutable `position`, closed state (`waiting`, `blocked`, `dispatched`, `running`), bounded retry count, visible blocked reason, enqueue actor/time, revision, and optional execution-request linkage. Dispatch resolves target and configuration at run time and uses `run_queue:<entryId>` idempotency. Queue mutation transactionally writes the resulting queued-first order through to `objectives.position`; `objectives.auto_advance` remains a transitional compatibility mirror.

### `execution_requests`

Durable queue for manual run, auto-advance, and — under
`requested_source = 'local_target_mutation'` — local-target **capability calls**
carried to another device (`metadata_json.overlord.localTargetMutation`).
...
| `workspace_id`                      | Id           | yes      | FK to `workspaces`.                                                                                                                                                                                                                                                                                                                                                                                    |
| `project_id`                        | Id           | yes      | FK to `projects`.                                                                                                                                                                                                                                                                                                                                                                                      |
| `mission_id`                        | Id           | cond.    | FK to `missions`. Required unless `requested_source = 'local_target_mutation'`, enforced by CHECK; a capability call with no mission (Latch probe, repository read, `doctor`) is authorized by `project_id` + `execution_target_id` instead.                                                                                                                                                             |
| `objective_id`                      | Id           | cond.    | FK to `objectives`. Same rule as `mission_id`; both are null together or both set.                                                                                                                                                                                                                                                                                                                     |
...
| `requested_source`                  | text         | yes      | `manual_run`, `auto_advance`, `api`, `cli`, `run_queue`, `local_target_mutation`, etc.                                                                                                                                                                                                                                                                                                                  |
| `idempotency_key`                   | text         | no       | Required for auto-advance and `run_queue`. A `local_target_mutation` may set it to a caller-supplied `operationId`, which makes a retried capability call resolve to the already-queued job.                                                                                                                                                                                                            |
...
| `metadata_json`                     | Json         | yes      | Core keys include `launchSession` (claim-time provider/viewer snapshot) and `providerSession` (external terminal-session mapping after Latch create: `{ provider, providerSessionId, executionTargetId, agentSessionId, createdAt, lastObservedState }`). Never Latch credentials, socket tokens, or attachment grants. `providerSession.providerSessionId` is never written to `launched_session_id`. |
| `created_at`                        | TimestampUTC | yes      |                                                                                                                                                                                                                                                                                                                                                                                                        |
| `updated_at`                        | TimestampUTC | yes      |                                                                                                                                                                                                                                                                                                                                                                                                        |
| `deleted_at`                        | TimestampUTC | no       | Tombstone.                                                                                                                                                                                                                                                                                                                                                                                             |
| `revision`                          | integer      | yes      |                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `mission.unblocked`      | A deliverable blocking question is resolved.                                                 |
...
- `/projects`, `/projects/:id/resources`, `/projects/:id/repository`, `/missions`, `/missions/:id/objectives`, `/missions/:id/events`, `/missions/:id/context`, `/missions/:id/deliveries`, `/workspaces/:id/objectives.csv`. Project resource create/update bodies accept additive `resourceKey`; objective DTOs and create/update bodies expose additive `resourceKey`. `ObjectiveDto` additively carries `displayKey` and computed `displayId` (`{mission.displayId}.{displayKey}`). `/api/objectives/:id` accepts an objective UUID or a full display id.
- `/api/projects/:id/run-queues` is the project-scoped Run Queue projection and definition surface. `GET` requires objective read; `POST`, `PATCH /api/projects/:id/run-queues/order`, `PATCH /api/run-queues/:queueId`, and `DELETE /api/run-queues/:queueId` require project update. Queue-order writes supply every live queue ID exactly once and only rewrite `run_queues.position`; entry membership/order remains exclusively in the Run Queue service. Entry enqueue, move, reorder, and dequeue retain `execution_request:create` authorization.
- `GET /api/missions/:id` additively projects `terminalSessions` from bounded `execution_requests.metadata_json.providerSession` mappings across all objective states. Each item includes the provider/session, execution request/objective/target, target/device label, stored session name, claim-time executable/viewer, creation time, and last observed `running` / `exited` / `stopping` / `lost` state. Reachability is a client-side local-target capability condition, not a persisted session state. The projection never includes launch environment, terminal output, credentials, socket tokens, or attachment grants.
