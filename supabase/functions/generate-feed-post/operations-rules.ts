...
export function formatCandidatesForPrompt(candidates: CandidateAction[]): string {
  ...
  return `CANDIDATE FOLLOW-UP ACTIONS (deterministically derived from repo structure — include relevant ones in human_actions, rephrase if needed, drop any that don't apply, and add others only if genuinely missing):\n${lines.join('\n')}`;
}
