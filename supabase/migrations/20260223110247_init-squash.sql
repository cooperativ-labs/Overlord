...
CREATE UNIQUE INDEX projects_org_everhour_project_id_key ON public.projects USING btree (organization_id, everhour_project_id) WHERE (everhour_project_id IS NOT NULL);
...
CREATE UNIQUE INDEX user_integrations_user_provider_key ON public.user_integrations USING btree (user_id, provider);
...
alter table "public"."projects" add constraint "projects_color_hex_check" CHECK ((color ~ '^#([0-9A-Fa-f]{6})$'::text)) not valid;

alter table "public"."projects" validate constraint "projects_color_hex_check";
...
alter table "public"."user_integrations" add constraint "user_integrations_user_provider_key" UNIQUE using index "user_integrations_user_provider_key";
