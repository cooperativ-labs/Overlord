...
- Follow-up draft creation accepts `projectId` / `--project-id` / `project_id` across API, CLI, hosted MCP, and the local Codex shim. When omitted, follow-up `create` inherits the current ticket project; when provided, it overrides that project explicitly.
