...
- API requires `agentIdentifier` and `connectionMethod` on attach/connect/prompt. The CLI defaults them to `{{agent.protocolAgent}}`/`cli`; the MCP tool defaults to `mcp`. Override with `--agent` / `--method` when calling from a different runtime.
...
- `permission-request` is invoked by the {{agent.productName}} permission hook installed by the bundle. Agents do not normally call it directly.
