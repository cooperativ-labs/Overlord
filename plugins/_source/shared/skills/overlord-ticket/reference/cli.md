...
- `user_follow_up` — only when the {{hookDescription}} is unavailable; the hook normally posts follow-ups to the activity feed
...
```bash
ovld protocol create --agent {{agent.protocolAgent}} --objectives-json '[{"objective":"Capture follow-up work from this repository"}]'
```

```bash
ovld protocol prompt --agent {{agent.protocolAgent}} --objectives-json '[{"objective":"Implement feature X"}]' --priority medium
```
